...
(function () {
  ...
  console.log('what it is yo');
  if (window) {
    window.Credulous = Credulous;
  }

  console.log("hurp durp");
  // Assume we have jQuery up and running, and Credulous
  var model = new Credulous({ labels: ['hurp', 'durp']})
    , topics
    , s
    , v
    ;
  ...
  function initBookmarklet() {
    console.log('calling initBookmarklet');
    function showRecommenedPosts(model) {
      var topics = $('table tbody')[2]
        ;
      $(topics).children().each(function(i, tr) {
        var post = postFromTr(tr)
          , label
          ;
        label = classifyWithModel(model, post);
        if (label === 'dislike') {
          $(tr).hide();
        }
      });
    ...
    function loadModelAndClassifyPosts(cb) {
      console.log('load and classify');
      $.getJSON('http://localhost:3000/model', function (dat, status, xhr) {
        console.log('json is ', dat);
        model.fromJSON(dat);
        console.log(model);
        return cb(model);
      });
    ...
    function classifyWithModel(model, post) {
    ...
    function postFromTr(tr) {
      var post = {}
        ;
      post.title = $(tr).find('.title a').text();
      post.url = $(tr).find('.title a').attr('href');
      post.submittedBy = $($(tr).find('.subtext a')[0]).text()
      return post;
    }

    console.log('attaching myBookmarklet');
    window.myBookmarklet = function () {
      console.log('calling myBookmarklet');
      loadModelAndClassifyPosts(showRecommenedPosts);
    }
    console.log(window.myBookmarklet);
  }
